function [field,smask]=sla_CreateCircularSlantField(fieldSize,theta_deg,orient_deg,...
...
% aperture_deg: the size of circular aparture in degrees, [deg]
...
% smask       : spatial mask of the circular slant field, [row,col]
%
% !!! NOTICE !!!
...
% Last Update: "2018-09-27 17:05:42 ban"
...
% remove exceed region so that the height field is a true circular object
smask=(r_ellipse>aperture_pix & r<aperture_pix);
field( smask )=fill_val;
smask=~smask; % flip 0 & 1

% create aparture
field(r>=aperture_pix)=outer_val;
smask(r>=aperture_pix)=0; % smask is mask, so the outer region of the aperture should be zero.
