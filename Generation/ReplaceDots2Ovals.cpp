// ReplaceDots2Ovals.c
//
// img=ReplaceDots2Ovals(img,dot_pos_idx,noise_dot_num,wdot,bdot,dotalpha,bgcolor,LR_flag)
//
// MEX function used in "sla_RDSfastest_with_noise_MEX" function
//
...
// Last Update: "2011-05-18 10:23:44 ban"
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
  ...
  UINT num_pos_dots=mxGetM(prhs[1]);

  // get the number of noise dots
  UINT num_noise=(UINT)mxGetScalar(prhs[2]);

  // get the size of white/black dots
  const int
      wdot_rows=mxGetM(prhs[3]), wdot_cols=mxGetN(prhs[3]),
      bdot_rows=mxGetM(prhs[4]), bdot_cols=mxGetN(prhs[4]);

  // get alpha value
  double *alpha_vals=(double *)mxGetData(prhs[5]);

  // get background color
  UINT bgcolor=(UINT)mxGetScalar(prhs[6]);

  // get left/right flag
  UINT left_flag=(UINT)mxGetScalar(prhs[7]);
  ...
  double *pos_ptr=mxGetPr(prhs[1]);
  double *wdot_ptr=mxGetPr(prhs[3]);
  double *bdot_ptr=mxGetPr(prhs[4]);
  ...
  for(i=0;i<num_pos_dots;i++){
    ...
    if(i<num_noise){
      ...
      //else{ dot_ptr=bdot_ptr; dot_rows=bdot_rows; dot_cols=bdot_cols; } // all black
    }
